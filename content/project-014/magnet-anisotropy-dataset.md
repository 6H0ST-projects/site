...
The established-magnet rows validate the sweep externally and set the scale for the robustness column. In the ideal corner of processing space the sweep recovers each compound's theoretical energy-product ceiling \((\mu_0 M_s)^2/4\mu_0\): 516 kJ/m³ for Nd₂Fe₁₄B, where sintered production magnets deliver ≈400; 220 for SmCo₅; and 46 kJ/m³ for the hexaferrite, close to its ≈40 commercial value. The robustness ordering matches processing history. SmCo₅ scores 0.84, and its κ = 4.4 margin is the reason the compound was manufacturable almost as soon as it was discovered. Nd₂Fe₁₄B scores 0.21: its higher ceiling comes with a narrower processing window, and widening that window is what decades of grain-boundary engineering in the sintered-magnet industry have pursued. The three zeros record compounds whose intrinsic ceilings sit below the table's thresholds. MnBi, MnAl-τ, and the hexaferrite serve lower-grade market segments; they score zero here because no microstructure can deliver a 0.8 T remanence from a saturation magnetization below it. Against these anchors, Mn₂SbTe's 0.71 places it between SmCo₅ and Nd₂Fe₁₄B in processing tolerance at half the ceiling, and the borides pair Nd₂Fe₁₄B-class ceilings with windows an order of magnitude narrower than Nd₂Fe₁₄B's own.
