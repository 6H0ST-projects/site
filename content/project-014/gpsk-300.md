...
**Table A4.** Per-checkpoint structure-match recovery on the seen and held-out L1₀ families, measured at N = 10 samples per prompt (the trajectory run plotted in Figure 15). The 400k checkpoint is the deployed model; its rates in Table 4 (58% seen, 48% held-out) come from the higher-precision N = 48 run, and the N = 10 row here sits within that run's sampling noise.
