...
We present GPSK-300, a 302M-parameter multimodal diffusion transformer that generates inorganic crystal structures in reciprocal space. Prior generative models encode crystals in real space and predict the lattice with a learned head, so any lattice error carries into the final structure. Our representation instead includes a reciprocal-metric channel whose quadratic form fixes the six lattice parameters, so a linear least-squares fit recovers the lattice in closed form while an inverse Fourier transform recovers the atomic positions; the model only has to generate a coherent grid, and the geometry follows by arithmetic. An ablation confirms the channel is essential: closed-form recovery reaches ≈0% lattice error against 7.7% for a learned head.
...
Generative models replace search with sampling. CDVAE [25] pairs a variational autoencoder with a diffusion decoder and reports reconstruction and de novo generation; MatterGen [10] diffuses atom types, coordinates, and the lattice jointly and conditions on target properties for de novo design; DiffCSP [26] and FlowMM [27] address CSP directly, the former with periodic equivariant diffusion and the latter with Riemannian flow matching, both scoring match rate against reference structures. All four encode the crystal in real space and produce the lattice with a learned component, so an error in the predicted lattice carries into the final structure. GPSK-300 differs in the representation: it generates in reciprocal space, where the lattice follows from the model output in closed form rather than from a learned head.
